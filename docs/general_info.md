...
* Степень приватности
  ...
  * **Полуоткрытое** - Указано описание мероприятия, но не время и место. Необходимо, чтобы организатор сам вас добавил в список участников. Но есть возможность отправить заявку на участие - "Постучаться". Организатору приходит в личку запрос. Он может посмотреть профиль и поболтать в чате. В этом же чате есть возможность подтвердить заявку. Пример - тусовка с незнакомыми людьми, но с возможностью предварительного отбора участников.
* Чат с участниками
  * Может быть отключён
* Рейтинг организатора
* У пользователя, как у организатора есть потоки событий. Именно к ним относятся мероприятия, которые организатор создаёт. Аналогично плейлисту на Youtube.
* Возможно подписаться на пользователя вцелом, либо только на плейлист.
...
* Создание потока мероприятий или добавление создаваемого события в уже существующий поток
...
* По подпискам на организаторов или потоки событий от организаторов. Схоже с персональными рекомендациями
...
* На странице другого пользоавтеля:
  * **Могут быть скрыты** ыНазвание потока, который он организует. Ниже карусель с конкретными мероприятиями этого потока.
  * **Могут быть скрыты** Подписки самого другого пользователя. Ниже карусель, где каждый квадратик - это чужой поток.
* Главная страница пользователя:
  * Мероприятия, куда согласился идти, и те, что сам организовываешь
  * Потоки, на которые подписан(аватарка - самое новое событие этого потока; при появлении нового мероприятия появляется синяя точка - индикатор мероприятия, которое пользователь не видел; название потока мероприятий; имя пользователя-организатора) и пользователи-организаторы на которых подписан(аватарка - аватарка пользователя; при появлении нового мероприятия появляется синяя точка - индикатор мероприятия, которое пользователь не видел; имя пользователя-организатора).
  * Актуальное в этом городе(могут быть проплаченные)
